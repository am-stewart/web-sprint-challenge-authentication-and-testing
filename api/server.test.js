...
describe('[POST], /api/auth/login', () => {
  it('should return username and password required message if either is not sent', async () => {
    await request(server).post('/api/auth/register').send({ username: 'abc', password: '123' })
    const res = await request(server).post('/api/auth/login').send({ username: '', password: '123' })
    expect(res.body).toEqual({ message: 'username and password required'})
  })
  it('should return invalid credentials message if invalid username and/or password', async () => {
    await request(server).post('/api/auth/register').send({ username: 'abc', password: '123' })
    let res = await request(server).post('/api/auth/login').send({ username: 'def', password: '123' })
    expect(res.body).toEqual({ message: 'invalid credentials'})
    res = await request(server).post('/api/auth/login').send({ username: 'abc', password: '456' })
    expect(res.body).toEqual({ message: 'invalid credentials'})
  })
  it('should return status code 200 after successful login', async () => {
    await request(server).post('/api/auth/register').send({ username: 'abc', password: '123' })
    const res = await request(server).post('/api/auth/login').send({ username: 'abc', password: '123' })
